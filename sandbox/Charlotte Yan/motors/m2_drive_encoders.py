...
def main():
    ...
    while time_s != 0:
        ...
        if sp == 0 or distance == 0:
            break
        degree_per_inch = 90
        motor_turns_needed_in_degrees = distance*degree_per_inch

        left_motor.run_to_rel_pos(position_sp = motor_turns_needed_in_degrees,speed_sp = sp,stop_action = ev3.Motor.STOP_ACTION_BRAKE)
        right_motor.run_to_rel_pos(position_sp=motor_turns_needed_in_degrees, speed_sp=sp, stop_action=ev3.Motor.STOP_ACTION_BRAKE)
        ev3.Sound.beep().wait()
